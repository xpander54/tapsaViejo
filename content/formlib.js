...
function processMultiselectsOnFormSubmit() {
  for (i=0; i<document.forms.length; i++) {
    ...
    for (j=0; j<fm.elements.length; j++) {
      ...
      // Process elements of XStandard XHML WYSIWYG Editor.
      if ( el.id.indexOf( 'xstandard_') == 0)
      {
          var elName = el.id.substring( el.id.indexOf( '_') + 1);
          document.getElementById('editor_'+elName).EscapeUnicode = true;
          document.getElementById('xstandard_'+elName).value = document.getElementById('editor_'+elName).value;
      }
